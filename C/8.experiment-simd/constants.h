...
// =============================================================================
// Współczynniki dla eliminacji dzieleń zmiennoprzecinkowych (Strength Reduction)
// =============================================================================

constexpr double INV_EV_TO_J        = 1.0 / EV_TO_J;              // Dżule -> eV
constexpr double INV_E_MASS         = 1.0 / E_MASS;               // 1 / m_e [1/kg]
constexpr double TWO_OVER_E_MASS    = 2.0 / E_MASS;               // 2.0 / m_e (dla prędkości ze straty energii)
constexpr double INV_AR_MASS        = 1.0 / AR_MASS;              // 1 / m_Ar [1/kg]
constexpr double INV_DE_CS          = 1.0 / DE_CS;                // 1 / DE_CS (1000.0)
constexpr double FACTOR_ENERGY_E    = (0.5 * E_MASS * INV_EV_TO_J) * INV_DE_CS; // v_sqr -> energy_index (elektrony)
constexpr double FACTOR_ENERGY_I    = (0.5 * MU_ARAR * INV_EV_TO_J) * INV_DE_CS; // g_sqr -> energy_index (jony)
constexpr double INV_DE_IFED        = 1.0 / DE_IFED;              // 1 / DE_IFED
constexpr double FACTOR_ENERGY_IFED = (0.5 * AR_MASS * INV_EV_TO_J) * INV_DE_IFED; // v_sqr -> ifed index
constexpr double INV_DE_EEPF        = 1.0 / DE_EEPF;              // 1 / DE_EEPF
constexpr double OPAL_FACTOR        = (1.0 / EV_TO_J) / 20.0;     // Rozkład Opla: energy -> arg atan