...
class HybridPoissonTest : public ::testing::Test {
protected:
    void SetUp() override { reset_state(); }
};

// Test A: potencjał liniowy w próżni (ρ=0)
TEST_F(HybridPoissonTest, VacuumLinearPotential) {
    xvector rho = {};
    solve_Poisson(rho, 0.0);   // tt=0 -> VOLTAGE*cos(0) = VOLTAGE

    for (int i = 0; i < N_G; i++) {
        double expected = VOLTAGE * (1.0 - (double)i / (N_G - 1));
        EXPECT_NEAR(pot[i], expected, 1e-11) << "Niezgodność potencjału w węźle i=" << i;
    }
}

// Test B: E-pole stałe w próżni (E = V0 / L)
TEST_F(HybridPoissonTest, VacuumConstantEfield) {
    xvector rho = {};
    ...
    for (int i = 1; i < N_G - 1; i++) {
        EXPECT_NEAR(efield[i], E_expected, 1e-8) << "Niezgodność E-pola w węźle i=" << i;
    }
}

// Test C: warunki brzegowe E-pola z niezerową gęstością rho na samych granicach
TEST_F(HybridPoissonTest, BoundaryEfieldWithCharge) {
    xvector rho = {};
    rho[0]     = 1e15 * E_CHARGE;
