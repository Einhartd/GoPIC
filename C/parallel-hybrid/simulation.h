...
inline void do_one_cycle (void){
    ...
    for (t=0; t<N_T; t++){          // the RF period is divided into N_T equal time intervals (time step DT_E)
        ...
        if ((t % 1000) == 0) {
            //  Collect number of electrons and ions from all mpi ranks
            //  and sum up them in master to show proper data
    ...
    //  Collect number of e and i from all mpi ranks
    //  and sum up in master rank
    int final_global_N_e = 0, final_global_N_i = 0;
    MPI_Reduce(&N_e, &final_global_N_e, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
    MPI_Reduce(&N_i, &final_global_N_i, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
    if (mpi_rank == 0) {
        fprintf(datafile,"%8d  %8d  %8d\n",cycle, final_global_N_e, final_global_N_i);
    }
}
