...
def step1_compute_electron_density(x_e, N_e, local_e_density:np.ndarray, e_density: np.ndarray, 
                                   cumul_e_density, INV_DX, FACTOR_W, N_G, num_threads):

    #   zerowanie buforow i wyjsciowej gestosci
...
@numba.njit(parallel=True, cache=True)
def step1_compute_ion_density(x_i, N_i, local_i_density:np.ndarray, i_density:np.ndarray, cumul_i_density,
                               INV_DX, FACTOR_W, N_G, t, N_SUB, num_threads):
    if t % N_SUB != 0:
...
def _do_one_cycle_jit(
    ...
    x_i, vx_i, vy_i, vz_i, N_i,
    e_density, i_density, cumul_e_density, cumul_i_density,
    ...
    for t in range(N_T):
        ...
        step1_compute_electron_density(
            x_e, N_e, local_e_density, e_density, cumul_e_density,
            INV_DX, FACTOR_W, N_G, num_threads
        )
        step1_compute_ion_density(
            x_i, N_i, local_i_density, i_density, cumul_i_density,
            INV_DX, FACTOR_W, N_G, t, N_SUB, num_threads
        )
        ...
        step9_collect_xt_data(
            pot, efield, e_density, i_density,
            pot_xt, efield_xt, ne_xt, ni_xt,
            N_G, t_index, measurement_mode
        )

        if t % 1000 == 0:
            print(" c =", cycle_number, "  t =", t, "  #e =", N_e, "  #i =", N_i)
...
def do_one_cycle(sim: SimulationState, datafile_path: str = "conv.dat"):
    """
    Executes a full RF cycle compiled inside JIT (_do_one_cycle_jit).
    """
    (
        ...
        sim.e_density, sim.i_density, sim.cumul_e_density, sim.cumul_i_density,
