...
def collision_electron (sim: SimulationState, k: int, e_index: int):
    ...
    #   Pobranie aktualnych wartosci czastki, zamiast wskaznikow
    xe: float = sim.x_e[k]
    ...
    if rnd < (t0 / t2):
        ...
        # (W C++ użyto nadpisania zmiennych gx, gy, gz, tu wprowadzamy lokalne gx2 dla przejrzystości)
        ...
        sim.x_i[sim.N_i] = xe
        sim.vx_i[sim.N_i] = sim.rng.gauss(0.0, cs.NORMAL_DISTRIBUTION)
        sim.vy_i[sim.N_i] = sim.rng.gauss(0.0, cs.NORMAL_DISTRIBUTION)
        sim.vz_i[sim.N_i] = sim.rng.gauss(0.0, cs.NORMAL_DISTRIBUTION)
        sim.N_i += 1
