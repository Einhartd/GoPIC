...
class SimulationState:
    ...
    def __init__(self, comm=None):
        ...
        """local electron density for this MPI rank (N_G)"""
        self.i_density: np.ndarray       = np.zeros(cs.N_G, dtype=np.float64)
        """local ion density for this MPI rank (N_G)"""
        self.global_e_density: np.ndarray = np.zeros(cs.N_G, dtype=np.float64)
        """global electron density across all MPI ranks (N_G)"""
        self.global_i_density: np.ndarray = np.zeros(cs.N_G, dtype=np.float64)
        """global ion density across all MPI ranks (N_G)"""
