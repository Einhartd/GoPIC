...
func doOneCycle() {
	const DV float64 = ELECTRODE_AREA * DX
	const FACTOR_W float64 = WEIGHT / DV
	const FACTOR_E float64 = DT_E / E_MASS * E_CHARGE
	const FACTOR_I float64 = DT_I / AR_MASS * E_CHARGE
	const MIN_X float64 = 0.45 * L // min. position for EEPF collection
	const MAX_X float64 = 0.55 * L // max. position for EEPF collection
	var k, t, p, energy_index int
	var g, g_sqr, gx, gy, gz, vx_a, vy_a, vz_a, e_x, energy, nu, p_coll, v_sqr, velocity float64
	var mean_v, c0, c1, c2, rate float64
	var out bool
	var rho xvector
	var t_index int
	...
	for t = 0; t < N_T; t++ { // the RF period is divided into N_T equal time intervals (time step DT_E)
		...
		solvePoisson(&rho, Time) // compute potential and electric field
