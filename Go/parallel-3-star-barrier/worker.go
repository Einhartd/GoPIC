...
func (sim *SimulationState) executeWorkerTask(workerID int, cmd WorkerCommand) {
	...
	case CmdCheckBoundariesElectrons:
		sim.workerCheckBoundariesElectrons(workerID)
	case CmdMoveIons:
		sim.workerMoveIons(workerID)
	case CmdCheckBoundariesIons:
		sim.workerCheckBoundariesIons(workerID)
	case CmdCollisionsE:
...
// workerMoveElectrons integruje ruch elektronów Leap-Frog (Krok 3).
func (sim *SimulationState) workerMoveElectrons(workerID int) {
	...
	if sim.Measurement_mode {
		*diag = electronWorkerDiagnostics{}
		...
		// Ścieżka szybka (bez pomiarów): rozwinięcie x4
		if end > start {
			_ = sim.X_e[end-1]
			_ = sim.Vx_e[end-1]
		}
...
// workerCheckBoundariesElectrons wykonuje równoległe sprawdzenie granic elektronów (Krok 5).
func (sim *SimulationState) workerCheckBoundariesElectrons(workerID int) {
	chunkSize := sim.EChunkSize
	if chunkSize <= 0 {
		chunkSize = (sim.N_e + sim.NumWorkers - 1) / sim.NumWorkers
	}
...
// workerMoveIons integruje ruch jonów Leap-Frog (Krok 4).
...
// workerCheckBoundariesIons wykonuje równoległe sprawdzenie granic jonów i diagnostykę IFED (Krok 6).
func (sim *SimulationState) workerCheckBoundariesIons(workerID int) {
	chunkSize := sim.IChunkSize
	if chunkSize <= 0 {
		chunkSize = (sim.N_i + sim.NumWorkers - 1) / sim.NumWorkers
	}
