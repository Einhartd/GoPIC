#!/bin/bash -l
#SBATCH --job-name=edupic_c_seq_rec
...
LOG_DIR="$(pwd)/saved_logs_C/logs_job_${SLURM_JOB_ID}_RECORD"
...
echo "=== [C++ Sequential RECORD] Job: ${SLURM_JOB_ID} | Threads: 1 (Allocated Cores: ${SLURM_CPUS_PER_TASK:-1}) | Cycles: ${N_CYCLES} | Measurement: ${MEASURE_ARG:-off} | Node: ${SLURM_JOB_NODELIST} ==="
...
if [ -f "${FLAME_DIR}/stackcollapse-perf.pl" ] && [ -f "${FLAME_DIR}/flamegraph.pl" ]; then
    ...
    perl "${FLAME_DIR}/flamegraph.pl" --title "C++ Sequential (Job ${SLURM_JOB_ID})" "${DATA_DIR}/perf.folded" > "${DATA_DIR}/flamegraph.svg" 2>/dev/null || true
fi
