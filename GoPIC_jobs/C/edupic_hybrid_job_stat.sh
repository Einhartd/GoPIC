...
echo "=== [C++ Hybrid STAT] Job: ${SLURM_JOB_ID} | MPI Tasks: ${SLURM_NTASKS} | OMP Threads: ${OMP_NUM_THREADS} | Cycles: ${N_CYCLES} | Measurement: ${MEASURE_ARG:-off} ==="
...
perf stat \
    ...
    mpirun --bind-to none -np "${SLURM_NTASKS}" "${BINARY}" "${N_CYCLES}" ${MEASURE_ARG}
