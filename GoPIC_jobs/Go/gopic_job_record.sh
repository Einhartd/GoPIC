...
LOG_DIR="$(pwd)/saved_logs_Go/logs_job_${SLURM_JOB_ID}_RECORD"
...
echo "=== [Go Sequential RECORD] Job: ${SLURM_JOB_ID} | Threads: 1 (Allocated Cores: ${SLURM_CPUS_PER_TASK:-1}) | Cycles: ${N_CYCLES} | Measurement: ${MEASURE_ARG:-off} | Node: ${SLURM_JOB_NODELIST} ==="
...
if [ "${USE_NC}" = "1" ] || [ "${USE_NC}" = "true" ]; then
    echo ">> Kompilacja: Go Sequential (Null-Collision, GOAMD64=${GOAMD64}, debug symbols zachowane)..."
    go build -tags nullcollision -o "${BINARY}" ./cmd/pic
else
    echo ">> Kompilacja: Go Sequential (Standard MCC, GOAMD64=${GOAMD64}, debug symbols zachowane)..."
...
if [ -f "${FLAME_DIR}/stackcollapse-perf.pl" ] && [ -f "${FLAME_DIR}/flamegraph.pl" ]; then
    ...
    perl "${FLAME_DIR}/flamegraph.pl" --title "Go Sequential (Job ${SLURM_JOB_ID})" "${DATA_DIR}/perf.folded" > "${DATA_DIR}/flamegraph.svg" 2>/dev/null || true
fi
