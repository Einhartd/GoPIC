...
LOG_DIR="$(pwd)/saved_logs_Go_opt/logs_job_${SLURM_JOB_ID}_OPTIMIZED_RECORD"
...
echo "=== [Go Optimized RECORD] Job: ${SLURM_JOB_ID} | Threads: ${GOMAXPROCS} (Allocated Cores: ${SLURM_CPUS_PER_TASK:-1}) | Workers: ${NUM_WORKERS} | Cycles: ${N_CYCLES} | Measurement: ${MEASURE_ARG:-off} | Node: ${SLURM_JOB_NODELIST} ==="
...
cd "${SRC_DIR}"
echo ">> Kompilacja: Go Optimized (StarBarrier, Loop Fusion, GOAMD64=${GOAMD64}, debug symbols zachowane dla perf)..."
...
if [ -d "${FLAME_DIR}" ]; then
    echo ">> Generowanie wykresu FlameGraph..."
    perf script -i "${PERF_DATA}" 2>/dev/null | "${FLAME_DIR}/stackcollapse-perf.pl" > "${DATA_DIR}/out.perf-folded" 2>&1 || true
    "${FLAME_DIR}/flamegraph.pl" "${DATA_DIR}/out.perf-folded" > "${DATA_DIR}/flamegraph.svg" 2>&1 || true
    rm -f "${DATA_DIR}/out.perf-folded"
fi
