...
## 6. Wyniki Telemetryczne z Klastra Lem HPC

Wszystkie pomiary Eksperymentu 8 wykonano na dedykowanym węźle `r11ch03b03` klastra Lem (AMD EPYC 9554, stałe taktowanie 3.68 GHz, 3 powtórzenia pomiarowe):

### 6.1. Tabela Zbiorcza: Ewolucja Metryk Hardware Counters (100 cykli RF)

| Metryka Perf | Eksperyment 1 (Baseline) | Eksperyment 6 (Euler) | Eksperyment 7 (Pusher & Bound.) | Eksperyment 8 (SIMD & Zen 4) | Delta (Exp 7 $\to$ Exp 8) | Skumulowany Zysk (1 $\to$ 8) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **Czas wykonania [s]** | 1099.4 s | 297.6 s (min 290.5 s) | 246.2 s (stabilne: 216.1 s) | **161.3 s** (min **160.4 s**) | **-54.8 s (-25.4% / -34.5%)** | **6.81x szybciej (-938 s)** |
| **Liczba cykli (cycles)** | 3 892.4 G | 1 093.4 G | 873.1 G (czyste: 800.0 G) | **593.9 G** | **-206.1 G (-25.8% / -32.0%)** | **-84.7% cykli CPU** |
| **Liczba instrukcji** | 12.107 T | 2.919 T | 3.013 T | **1.967 T** | **-1.046 T (-34.7%)** | **-10.14 T (-83.8%)** |
| **Instrukcje na cykl (IPC)** | 3.11 | 2.67 | 3.49 (czyste: 3.76) | **3.31** | *(wektoryzacja 512-bit ZMM)* | wysokie nasycenie potoków |
| **Liczba skoków (branches)** | 1 168.2 G | 252.6 G | 252.7 G | **168.4 G** | **-84.3 G (-33.4%)** | **-85.6% gałęzi** |
| **Błędy skoków (branch-misses)**| 1 860.8 M (0.16%)| 172.3 M (0.07%) | 171.6 M (0.07%) | **166.9 M (0.10%)** | -4.7 M | niemal zerowy stall skoków |
| **Odczyty L1 (L1-loads)** | 2 876.0 G | 572.2 G | 690.7 G | **464.0 G** | **-226.7 G (-32.8%)** | **-83.9% dostępów do L1** |
| **Nietrafienia L1 (L1-misses)**| 57.2 G (1.99%) | 31.5 G (5.50%) | 31.5 G (4.56%) | **29.4 G (6.33%)** | -2.1 G mniej missów | kompaktowy footprint cache |

> **Kluczowe obserwacje mikroarchitektoniczne:**  
> 1. **Redukcja ponad 1 BILIONA instrukcji (-34.7%):** Przejście na architekturę AVX-512 (`-mprefer-vector-width=512`, `%zmm`) pozwoliło przetwarzać 8 liczb podwójnej precyzji w jednej instrukcji, co obcięło 1.046 biliona instrukcji w 100 cyklach RF.  
> 2. **33% redukcji skoków:** 4-krotne rozwinięcie pętli integratora (`k += 4`) skróciło liczbę wykonanych instrukcji rozgałęzień z 252.7 G do 168.4 G.  
> 3. **Spadek obciążenia L1 Data Cache o 226 miliardów odczytów:** Dyrektywa `alignas(64)` i 512-bitowe wektory zredukowały liczbę dostępów do L1D o 32.8%, całkowicie eliminując kary za rozcinanie linii cache (*cache-line splits*).  
> 4. **Zrównanie z referencyjnym pomiarem 1-wątkowym OpenMP:** Czas 161.3 s i 1.967 T instrukcji bezpośrednio wyjaśnia tajemnicę wyniku `OMP_STAT-1_1` (148.2 s, 1.868 T instrukcji) – implementacja OMP zawierała już te same techniki wektorowe i flagi architektoniczne.

---

## 7. Weryfikacja Fizyczna i Niezmienniki Plazmy (Golden Record Validation)

Kod `C/8.experiment-simd` przeszedł pełną, rygorystyczną procedurę weryfikacyjną `run_verify.sh` w środowisku pomiarowym (5 cykli RF od cyklu 2001 do 2006):

```
Electron density @ center             = 7.538e+15 [m^{-3}]
...
Liczba elektronów w układzie          = 108 175
Liczba jonów w układzie               = 113 606
