...
## 4. Wyniki Telemetryczne z Klastra Lem HPC

### 4.1. Tabela Zbiorcza Metryk Hardware Counters (100 cykli RF)

Pomiary wykonano na węzłach obliczeniowych klastra Lem (procesory AMD EPYC 9554 64-Core, 3.67–3.70 GHz). W tabeli zestawiono uśrednione wyniki trzech powtórzeń (z wyszczególnieniem węzłów bez zakłóceń rywalizacji magistrali):

| Metryka Perf | Eksperyment 5 (Fast Path MCC) | Eksperyment 6 (Euler Vector) | Eksperyment 7 (Pusher & Boundaries) | Delta (Exp 6 $\to$ Exp 7) |
| :--- | :---: | :---: | :---: | :---: |
| **Czas wykonania [s]** | 256.7 s (min 241.1 s) | 297.6 s (min 290.5 s) | **246.2 s** (stabilne: **216.1 s**) | **-51.4 s (-17.3%)** *(na czystym: -74.4 s / -25.6%)* |
| **Liczba cykli (cycles)** | 919.8 G | 1 093.4 G | **873.1 G** (czyste: **800.0 G**) | **-220.3 G (-20.1%)** *(czyste: -273.4 G / -25.5%)* |
| **Liczba instrukcji** | 2.957 T | 2.919 T | **3.013 T** | +0.094 T (+3.2%) |
| **Instrukcje na cykl (IPC)** | 3.22 | 2.67 | **3.49** (czyste: **3.76**) | **+0.82 (+30.7%)** *(czyste: +1.04 / +38.3%)* |
| **Liczba skoków (branch-loads)** | 258.3 G | 252.6 G | **252.7 G** | ~0.0% |
| **Błędy predykcji (branch-misses)**| 373.2 M (0.14%) | 172.3 M (0.07%) | **171.6 M (0.07%)** | -0.7 M |
| **Odczyty L1 (L1-dcache-loads)** | 585.6 G | 572.2 G | **690.7 G** | +118.5 G (buforowanie dwuetapowe) |
| **Nietrafienia L1 (L1-load-misses)**| 31.6 G (5.40%) | 31.5 G (5.50%) | **31.5 G (4.56%)** | Stopa błędów L1 spadła o 0.94 p.p. |

> **Interpretacja mikroarchitektoniczna:**  
> Pomimo minimalnego wzrostu bezwzględnej liczby instrukcji (+3.2% wynikające z zarządzania wektorem `absorbed_indices` w fazie 1 sprawdzania granic), czas wykonania uległ drastycznemu skróceniu o **ponad 51–74 sekundy**, a wskaźnik IPC wzrósł z **2.67 aż do 3.76** (rekordowa wartość w części sekwencyjnej na generycznych flagach).  
> Kluczowym mechanizmem jest **usunięcie rozgałęzienia diagnostycznego `if (measurement_mode)` z 43 miliardów iteracji pushera**, co pozwoliło procesorowi na pełne potokowanie instrukcji zmiennoprzecinkowych FMA bez konieczności spekulatywnego wykonywania kodu przez jednostkę predykcji skoków. Dodatkowo, zamiana pętli `while (k < N)` ze `swap-with-last` na liniową, deterministyczną pętlę `for` ustabilizowała dostęp do pamięci L1D (spadek stopy nietrafień do 4.56%).
